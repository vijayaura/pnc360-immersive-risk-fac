...
export const EndorsementsTab = React.memo(({ itemsPerPage, isActive, selectedProductIds }: EndorsementsTabProps) => {
    ...
    const visibleEndorsementColumns = getTableVisibility('insurer-endorsements', endorsementColumns.map(c => c.id));
    ...
    useEffect(() => {
        if (endorsementSearchDebounceRef.current) clearTimeout(endorsementSearchDebounceRef.current);
        const delay = endorsementSearch.trim() ? 300 : 0;
        endorsementSearchDebounceRef.current = setTimeout(() => {
            setDebouncedEndorsementSearch(endorsementSearch);
            setEndorsementPage(1);
        }, delay);
        return () => {
            if (endorsementSearchDebounceRef.current) clearTimeout(endorsementSearchDebounceRef.current);
    ...
    const getEndorsementAction = useCallback((status: Status, endorsementId: string) => {
        const isFinalizedStatus = status === 'Approved' || status === 'Rejected';

        return isFinalizedStatus
            ? {
                label: 'View',
                icon: <Eye className="w-4 h-4 mr-1" />,
                href: `/insurer/endorsements/view/${endorsementId}`,
            }
            : {
                label: 'Edit',
                icon: <Edit className="w-4 h-4 mr-1" />,
                href: `/insurer/endorsements/edit/${endorsementId}`,
            };
    }, []);
                ...
                <div className="search-filter-container mb-6 flex items-start gap-3">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3 flex-1">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                onChange={(e) => {
                                    setEndorsementSearch(e.target.value);
                                }}
                            />
                        </div>
                        <Select
                        ...
                        >
                            <SelectTrigger className="bg-background">
                                <span className="truncate">
                                    {endorsementStatusFilter === 'all-statuses' ? 'All Statuses' :
                        ...
                        >
                            <SelectTrigger className="bg-background">
                                <span className="truncate">
                                    {endorsementTypeFilter === 'all-types' ? 'All Types' :
                                        endorsementTypeFilter === 'technical' ? 'Financial' :
                                            endorsementTypeFilter === 'non_technical' ? 'Non-Financial' :
                                                endorsementTypeFilter === 'extensions' ? 'Extensions' :
                                                'Cancellation'}
                                </span>
                            </SelectTrigger>
                            <SelectContent>
                    ...
                    <div className="ml-auto pt-0.5">
                      <ColumnVisibilityDropdown
                        ...
                        onToggleColumn={(id) => toggleColumnVisibility('insurer-endorsements', id, endorsementColumns.map(c => c.id))}
                        onReset={() => setColumnVisibility('insurer-endorsements', endorsementColumns.map(c => c.id))}
                      />
                    </div>
                </div>
                            ...
                            {endorsementRows.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={visibleEndorsementColumns.length} className="text-center text-muted-foreground py-8">
                                        {endorsementSearch ? 'No endorsements match your search.' : 'No endorsements found.'}
                                    </TableCell>
                                </TableRow>
                            ) : (
                                endorsementRows.map((end) => {
                                    const action = getEndorsementAction(end.status, end.id);

                                    return (
                                        <TableRow key={end.id} className="hover:bg-muted/50">
                                            {visibleEndorsementColumns.includes('reference') && <TableCell className="font-medium whitespace-nowrap">{end.endorsementReference}</TableCell>}
                                            {visibleEndorsementColumns.includes('policyNumber') && <TableCell className="whitespace-nowrap">{end.policyNumber}</TableCell>}
                                            {visibleEndorsementColumns.includes('type') && (
                                                <TableCell className="whitespace-nowrap">
                                                    <span className="text-sm">{end.endorsementType}</span>
                                                </TableCell>
                                            )}
                                            {visibleEndorsementColumns.includes('createdDate') && (
                                                <TableCell className="whitespace-nowrap">
                                                    {end.createdAt ? format(end.createdAt, 'dd-MM-yyyy') : '-'}
                                                </TableCell>
                                            )}
                                            {visibleEndorsementColumns.includes('effectiveDate') && (
                                                <TableCell className="whitespace-nowrap">
                                                    {end.effectiveDate ? format(end.effectiveDate, 'dd-MM-yyyy') : '-'}
                                                </TableCell>
                                            )}
                                            {visibleEndorsementColumns.includes('premium') && <TableCell className="whitespace-nowrap">{formatPremiumAmount(end.totalEndorsementAmount)}</TableCell>}
                                            ...
                                            {visibleEndorsementColumns.includes('messages') && (
                                                <TableCell 
                                                    ...
                                                    onClick={() => navigate(`${action.href}${action.href.includes('?') ? '&' : '?'}openMessage=true`)}
                                                >
                ...
                {endorsementMeta && (
                    <div className="px-6 py-4 border-t flex justify-between items-center">
                        <div className="text-sm text-muted-foreground">
                            {`Page ${endorsementMeta.page ?? endorsementPage} of ${endorsementMeta.totalPages || 1} · ${endorsementMeta.total} endorsements`}
                        </div>
                                ...
                                <PaginationContent>
                                    <PaginationItem>
                                        <PaginationPrevious
                                            ...
                                            className={
                                                endorsementPage === 1 ? 'pointer-events-none opacity-50' : ''
                                            }
                                        />
                                    </PaginationItem>
                                    {(() => {
                                    ...
                                    <PaginationItem>
                                        <PaginationNext
                                            ...
                                            onClick={(e) => {
                                                e.preventDefault();
                                                if (endorsementPage < endorsementMeta.totalPages)
                                                    setEndorsementPage(endorsementPage + 1);
                                            }}
                                            className={
                                                endorsementPage === endorsementMeta.totalPages
