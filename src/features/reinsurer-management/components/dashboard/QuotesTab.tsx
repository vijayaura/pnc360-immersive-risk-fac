...
import { getReferralDetail } from '@/features/proposals/api/referrals';
...
function getStatusBadge(status: string) {
  ...
  if (s === 'under_review' || s === 'under review')
    return <Badge variant="outline" className="text-gray-600 border-gray-300 bg-gray-50">Under Review</Badge>;
  if (s === 'approved' || s === 'accepted')
    return <Badge variant="outline" className="text-green-700 border-green-300 bg-green-50">Approved</Badge>;
  if (s === 'submitted')
    return <Badge variant="outline" className="text-blue-600 border-blue-300 bg-blue-50">Submitted</Badge>;
  if (s === 'draft')
    return <Badge variant="outline" className="text-slate-600 border-slate-300 bg-slate-50">Draft</Badge>;
  if (s === 'bound')
    return <Badge variant="outline" className="text-emerald-700 border-emerald-300 bg-emerald-50">Bound</Badge>;
  if (s === 'active')
...
interface QuotesTabProps {
  ...
  coverToProductIds?: Record<string, string[]>;
  gradeOptions?: { id: string; valueLabel: string }[];
}

export function QuotesTab({ filters, reinsurerOptions, brokerOptions, gradeOptions }: QuotesTabProps) {
  ...
  // Load all quote-pipeline allocations (include pre-policy rows)
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    const timeout = setTimeout(() => {
      fetchAllReinsurancePoliciesPages({
        ...
        .catch((err) => {
          console.error('[QuotesTab] Failed to fetch quotes:', err);
  ...
  const phaseFiltered = useMemo(() => {
    const allowed = new Set(QUOTE_STATUSES.map((s) => normalizeStatus(s)));
    return allListRows.filter((r) => allowed.has(normalizeStatus(r.status || '')));
  }, [allListRows]);
  ...
  useEffect(() => {
    ...
    batchEnrichPolicyRecordsForFilters(phaseFiltered, filters)
      ...
      .catch((err) => {
        console.error('[QuotesTab] Enrichment failed:', err);
  ...
  useEffect(() => {
    ...
    const fetchBrokerForVisibleRows = () => {
      const rowsNeedingBroker = pageRecords.filter((r) => {
        const cached = detailCache[r.id];
        const hasBroker = (cached?.brokerNames && cached.brokerNames.length > 0)
          || (r.brokerNames && r.brokerNames.length > 0);
        return !hasBroker && (r.referralId || cached?.referralId);
      });
      if (rowsNeedingBroker.length === 0) return;

      Promise.allSettled(
        rowsNeedingBroker.map((r) => {
          const refId = r.referralId || detailCache[r.id]?.referralId;
          return getReferralDetail(refId!);
        }),
      ).then((brokerResults) => {
        if (!isMounted) return;
        setDetailCache((prev) => {
          const next = { ...prev };
          brokerResults.forEach((res, idx) => {
            const record = rowsNeedingBroker[idx];
            if (res.status === 'fulfilled' && res.value?.brokerName) {
              next[record.id] = {
                ...(next[record.id] ?? record),
                brokerNames: [res.value.brokerName],
              };
            }
          });
          return next;
  ...
  // Merge detail-enriched data into page rows, preserving list record's createdDateIso
      ...
      <CardContent className="pt-6">
        <div className="mb-4">
          ...
          <p className="text-sm text-muted-foreground">
            Quote-stage cases with reinsurance. Filters apply across the full list.
          </p>
          {enriching && (
          ...
          <TableBody>
            {showTableSkeleton ? (
              ...
              displayRecords.flatMap((r) => {
                ...
                const grossPremium = r.totals?.grossPremium ?? 0;
                const cessionPct = grossPremium > 0 ? Math.round((cession / grossPremium) * 100) : 0;

                const isUuid = (s: string) => /^[0-9a-f]{8}-[0-9a-f]{4}-/.test(s);
                const caseId = r.quoteNumber ?? (r.policyOrQuoteId && !isUuid(r.policyOrQuoteId) ? r.policyOrQuoteId : null);
                ...
                const rows = [
                  <TableRow key={r.id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => toggleRow(r.id)}>
                    ...
                    <TableCell className="font-medium whitespace-nowrap">{caseId ?? <Skeleton className="h-4 w-24" />}</TableCell>
                    ...
                    <TableCell className="text-right whitespace-nowrap">
                      {grossPremium > 0 ? formatCurrencyCompact(grossPremium) : '-'}
                    </TableCell>
